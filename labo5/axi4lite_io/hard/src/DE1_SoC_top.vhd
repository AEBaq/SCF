...
architecture top of DE1_SoC_top is
    ...
    component qsys_system is
        port (
            ...
            -- AXI4-Lite
            axi4lite_slave_input_buttons    : in    std_logic_vector(31 downto 0) := (others => 'X'); -- input_reg_a_i
            axi4lite_slave_input_switches   : in    std_logic_vector(31 downto 0) := (others => 'X'); -- input_reg_b_i
            axi4lite_slave_output_leds      : out   std_logic_vector(31 downto 0);                    -- output_reg_a_o
            axi4lite_slave_output_hex_3_0   : out   std_logic_vector(31 downto 0);                    -- output_reg_b_o
            axi4lite_slave_output_hex_5_4   : out   std_logic_vector(31 downto 0)                     -- output_reg_c_o
        );
    end component qsys_system;

    --| Signals declarations |--------------------------------------------------------------
    signal button_data_s  : std_logic_vector(31 downto 0);
    signal switch_data_s  : std_logic_vector(31 downto 0);
    signal led_data_s     : std_logic_vector(31 downto 0);
    signal disp_lower_s   : std_logic_vector(31 downto 0);
    signal disp_upper_s   : std_logic_vector(31 downto 0);
...
---------------------------------------------------------
    ...
    port map (
        ...
        -- AXI4-Lite
        axi4lite_slave_input_buttons    => button_data_s,
        axi4lite_slave_input_switches   => switch_data_s,
        axi4lite_slave_output_leds      => led_data_s,
        axi4lite_slave_output_hex_3_0   => disp_lower_s,
        axi4lite_slave_output_hex_5_4   => disp_upper_s
    );

    -- Input assignments
    button_data_s(KEY_i'range) <= not KEY_i;
    button_data_s(button_data_s'high downto KEY_i'high+1) <= (others => '0');

    switch_data_s(SW_i'range) <= SW_i;
    switch_data_s(switch_data_s'high downto SW_i'high+1) <= (others => '0');

    -- Output assignments
    LEDR_o <= led_data_s(LEDR_o'range);

    HEX0_o <= not disp_lower_s(6 downto 0);
    HEX1_o <= not disp_lower_s(14 downto 8);
    HEX2_o <= not disp_lower_s(22 downto 16);
    HEX3_o <= not disp_lower_s(30 downto 24);
    HEX4_o <= not disp_upper_s(6 downto 0);
    HEX5_o <= not disp_upper_s(14 downto 8);
